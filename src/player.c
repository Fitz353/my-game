...
void PlayerUpdate(Player* player, const bool* keys, float dt){
        ...
        if (can_left) {
            player->x -= speed;
            player->last_facing = FACING_LEFT;
        }
        if (can_right) {
            player->x += speed;
            player->last_facing = FACING_RIGHT;
        }
        if (can_up) {
            player->y -= speed;
            player->last_facing = FACING_UP;
        }
        if (can_down) {
            player->y += speed;
        ...
        else if (can_up && shift)   new_state = STATE_RUN_UP;
        else if (can_down && shift) new_state = STATE_RUN_DOWN;
        else if (can_left)          new_state = STATE_WALK_LEFT;
        else if (can_right)         new_state = STATE_WALK_RIGHT;
        else if (can_up)            new_state = STATE_WALK_UP;
        else if (can_down)          new_state = STATE_WALK_DOWN;
        ...
        if (player->x + player->col_offset_x < 0) {
            player->x = 0 - player->col_offset_x;
        }
        if (player->y + player->col_offset_y < 0) {
            player->y = 0 - player->col_offset_y;
        }
        if (player->x + player->col_offset_x + player->col_width > WIDTH) {
            player->x = WIDTH - player->col_width - player->col_offset_x;
        }
        if (player->y + player->col_offset_y + player->col_height > HEIGHT) {
            player->y = HEIGHT - player->col_height - player->col_offset_y;
        }
...
void PlayerRender(Player* player, SDL_Renderer* renderer) {
        SDL_FRect src = {player->current_frame * 64, player->current_row * 64,
  64, 64};
        ...
        // Debug Red Box
        SDL_FRect debug_col = {
            player->x + player->col_offset_x,
            player->y + player->col_offset_y,
            player->col_width,
            player->col_height
        };
